...
def soft_constraints(schedule, courses, schedule_counter):
    ...
    while schedule_counter < CYCLES:
        ...
        session_to_switch = -1
        schedule2 = Constraint.switch_session(schedule, 1, session_to_switch, courses)
        ...
        if accept_counter > OPTIMUM:
            # Lower the heat by making the optimum

            OPTIMUM += 2
            # Save first schedule at the optimum
            if save_first is False:
                save_schedule = schedule
                # First schedule is saved so set save_first on True
                save_first = True
            ...
            schedule = Constraint.switch_session(schedule, 1, session_to_switch, courses)
